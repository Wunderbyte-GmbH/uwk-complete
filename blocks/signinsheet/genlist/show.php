...
function buildMenu($cid){
	...
	$orderby = '';
    // Modified by Patrick Thibaudeau: Original value firstname
    ...
    if ($scheduleid) {
        $outputhtml = '<div style="float:left"><a href="' . $CFG->wwwroot . '/local/yulearn/admin/registration.php?id=' . $scheduleid . '" class="btn btn-outline-primary">';
        $outputhtml .= get_string('view_registered', 'local_yulearn');
        $outputhtml .= '</a></div>';
    } else {
        $outputhtml = '';
    }
