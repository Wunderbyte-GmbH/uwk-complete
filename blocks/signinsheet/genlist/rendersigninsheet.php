...
function renderGroup($extra){
	...
	$appendorder = '';
    // Modified by Patrick Thibaudeau: deafualt was blank
...
//	$outputhtml .= '<span style="font-size:18px"> <b>'. get_string('description', 'block_signinsheet').': __________________________________________________</b> </span><p></p>&nbsp;<p></p>&nbsp;';
	...
	$outputhtml .= '<table style="border-style: solid;" width="100%"  border="1px"><tr>
					<td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="20%"><b>'. get_string('personname', 'block_signinsheet').'</b></td>
				    <td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="20%"><b>'. get_string('email', 'block_signinsheet').'</b></td>
				    <td style="border-right: thin solid; border-bottom: thin solid" border="1px" width="30%"><b>'. get_string('department', 'block_signinsheet').'</b></td>';
...
function renderAll($extra){
	...
	$orderby = '';
	$cid = required_param('cid', PARAM_INT);
    // Modified by Patrick Thibaudeau: default was blank
	$orderby = optional_param('orderby', 'lastname', PARAM_TEXT);
	...
	$outputhtml .= '<table style="border-style: solid;" width="100%"  border="1px"><tr>
					<td style="border-right: thin solid; border-bottom: thin solid" border="1px"><b>'. get_string('personname', 'block_signinsheet').'</b></td>';
...
function printSingleUser($uid, $cid){
    ...
    $sql = "Select
                ...
                yd.name
            From
                {yulearn_employee} ye Inner Join
                {yulearn_department} yd On yd.id = ye.departmentid Inner Join
                {yulearn_registration} yr On ye.id = yr.employeeid
            Where
                yr.scheduleid = ? And
                yr.userid = ?";

    if ($result = $DB->get_record_sql($sql, [$scheduleid, $uid])) {
	...
	$outputhtml =  '
				...
				<tr height="10">
					<td  style="border-right: thin solid;  border-bottom: thin solid" border="1px" width="20%">' . $lastname . ', ' . $firstname  . '</td>'
                    . '<td style="border-right: thin solid;  border-bottom: thin solid" border="1px">' . $email . '</td>'
                    . '<td style="border-right: thin solid;  border-bottom: thin solid" border="1px">' . $department . '</td>';
