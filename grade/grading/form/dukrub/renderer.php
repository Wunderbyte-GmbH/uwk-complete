...
/**
 * Contains renderer used for displaying dukrub
 *
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class gradingform_dukrub_renderer extends plugin_renderer_base {

    /**
     * This function returns html code for displaying criterion. Depending on $mode it may be the
     * code to edit dukrub, to preview the dukrub, to evaluate somebody or to review the evaluation.
     *
     * This function may be called from display_dukrub() to display the whole dukrub, or it can be
     * called by itself to return a template used by JavaScript to add new empty criteria to the
     * dukrub being designed.
     ...
     *
     * @param int $mode dukrub display mode, see {@link gradingform_dukrub_controller}
     * @param array $options display options for this dukrub, defaults are: {@link gradingform_dukrub_controller::get_default_options()}
    ...
    public function criterion_template($mode, $options, $elementname = '{NAME}', $criterion = null, $levelsstr = '{LEVELS}', $value = null) {
        ...
        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
            ...
            $descriptiontextareaparams = array(
                ...
                'aria-label' => get_string('criterion', 'gradingform_dukrub', ''),
                'cols' => '10', 'rows' => '5'
            );
            $description = html_writer::tag('textarea', s($criterion['description']), $descriptiontextareaparams);
        } else {
            if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN) {
        ...
        if ($mode != gradingform_dukrub_controller::DISPLAY_EDIT_FULL &&
            $mode != gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN) {
            // Set description's cell as tab-focusable.
            $descriptiontdparams['tabindex'] = '0';
            // Set label for the criterion cell.
            $descriptiontdparams['aria-label'] = get_string('criterion', 'gradingform_dukrub', s($criterion['description']));
        }
        ...
        $levelstableparams = array(
            ...
            'aria-label' => get_string('levelsgroup', 'gradingform_dukrub')
        );
        ...
        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('criterionaddlevel', 'gradingform_dukrub');
        ...
        $displayremark = ($options['enableremarks'] && ($mode != gradingform_dukrub_controller::DISPLAY_VIEW || $options['showremarksstudent']));
        if ($displayremark) {
            $currentremark = '';
            ...
            $remarklabeltext = get_string('criterionremark', 'gradingform_dukrub', $remarkinfo);
            ...
            } else if ($mode == gradingform_dukrub_controller::DISPLAY_EVAL_FROZEN) {
            ...
            }else if ($mode == gradingform_dukrub_controller::DISPLAY_REVIEW || $mode == gradingform_dukrub_controller::DISPLAY_VIEW) {
     ...
     * it may be the code to edit dukrub, to preview the dukrub, to evaluate somebody or to review the evaluation.
     *
     * This function may be called from display_dukrub() to display the whole dukrub, or it can be
     * called by itself to return a template used by JavaScript to add new empty level to the
     * criterion during the design of dukrub.
    ...
    public function level_template($mode, $options, $elementname = '{NAME}', $criterionid = '{CRITERION-id}', $level = null) {
        ...
        if (!isset($level['id'])) {
            ...
            foreach (array('score', 'definition', 'class', 'checked', 'index', 'leveloverwrite') as $key) {
                // set missing array elements to empty strings to avoid warnings
        ...
        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
            $definitionparams = array(
                ...
                'aria-label' => get_string('leveldefinition', 'gradingform_dukrub', $levelindex),
                'cols' => '10', 'rows' => '4'
            );
            ...
            $scoreparams = array(
                ...
                'aria-label' => get_string('scoreinputforlevel', 'gradingform_dukrub', $levelindex),
                'size' => '3',
                'value' => $level['score']
            );
        ...
        if ($mode == gradingform_dukrub_controller::DISPLAY_EVAL_FROZEN && $level['checked']) {
        ...
        if (($mode == gradingform_dukrub_controller::DISPLAY_REVIEW || $mode == gradingform_dukrub_controller::DISPLAY_VIEW) && $level['checked']) {
            $score = (float)$level['leveloverwrite']; // Update view for graded students, display score.
        }
        ...
        if ($mode != gradingform_dukrub_controller::DISPLAY_EDIT_FULL &&
            ...
            $tdattributes['aria-label'] = get_string('level', 'gradingform_dukrub', $levelinfo);

            if ($mode != gradingform_dukrub_controller::DISPLAY_PREVIEW &&
                $mode != gradingform_dukrub_controller::DISPLAY_PREVIEW_GRADED) {
        ...
        $displayscore = true;
        if (!$options['showscoreteacher'] && in_array($mode, array(gradingform_dukrub_controller::DISPLAY_EVAL, gradingform_dukrub_controller::DISPLAY_EVAL_FROZEN, gradingform_dukrub_controller::DISPLAY_REVIEW))) {
            $displayscore = false;
        }
        if (!$options['showscorestudent'] && in_array($mode, array(gradingform_dukrub_controller::DISPLAY_VIEW, gradingform_dukrub_controller::DISPLAY_PREVIEW_GRADED))) {
            $displayscore = false;
        }
        if ($displayscore) {
            $scoreclass = 'score';
            ...
            $leveltemplate .= html_writer::tag('div', get_string('scorepostfix', 'gradingform_dukrub', $score), array('class' => $scoreclass));
        }
        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('leveldelete', 'gradingform_dukrub', $levelindex);
            $buttonparams = array(
                'type' => 'submit',
        ...
        if ($mode == gradingform_dukrub_controller::DISPLAY_EVAL) {
            ...
            // Removed: 'type' => 'number',
            $params = array(
                'name' => '{NAME}[criteria][{CRITERION-id}][leveloverwrite][{LEVEL-id}]',
                'id' => '{NAME}-criteria-{CRITERION-id}-leveloverwrite{LEVEL-id}',
                'style' => 'width:100%',
                'value' => $value
            );
    ...
    /**
     * This function returns html code for displaying dukrub template (content before and after
     * criteria list). Depending on $mode it may be the code to edit dukrub, to preview the dukrub,
     * to evaluate somebody or to review the evaluation.
     *
     * This function is called from display_dukrub() to display the whole dukrub.
     *
     * When overriding this function it is very important to remember that all elements of html
     * form (in edit or evaluate mode) must have the name $elementname.
     *
     ...
     * @param array $options display options for this dukrub, defaults are: {@link gradingform_dukrub_controller::get_default_options()}
     * @param string $elementname the name of the form element (in editor mode) or the prefix for div ids (in view mode)
     * @param string $criteriastr evaluated templates for this dukrub's criteria
     * @return string
     */
    protected function dukrub_template($mode, $options, $elementname, $criteriastr) {
        $classsuffix = ''; // CSS suffix for class of the main div. Depends on the mode
        switch ($mode) {
            case gradingform_dukrub_controller::DISPLAY_EDIT_FULL:
                $classsuffix = ' editor editable'; break;
            case gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN:
                $classsuffix = ' editor frozen';  break;
            case gradingform_dukrub_controller::DISPLAY_PREVIEW:
            case gradingform_dukrub_controller::DISPLAY_PREVIEW_GRADED:
                $classsuffix = ' editor preview';  break;
            case gradingform_dukrub_controller::DISPLAY_EVAL:
                $classsuffix = ' evaluate editable'; break;
            case gradingform_dukrub_controller::DISPLAY_EVAL_FROZEN:
                $classsuffix = ' evaluate frozen';  break;
            case gradingform_dukrub_controller::DISPLAY_REVIEW:
                $classsuffix = ' review';  break;
            case gradingform_dukrub_controller::DISPLAY_VIEW:
                $classsuffix = ' view';  break;
        }

        $dukrubtemplate = html_writer::start_tag('div', array('id' => 'dukrub-{NAME}', 'class' => 'clearfix gradingform_dukrub'.$classsuffix));

        // Dukrub table.
        $dukrubtableparams = array(
            'class' => 'criteria',
            'id' => '{NAME}-criteria',
            'aria-label' => get_string('dukrub', 'gradingform_dukrub'));
        $dukrubtable = html_writer::tag('table', $criteriastr, $dukrubtableparams);
        $dukrubtemplate .= $dukrubtable;
        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
            $value = get_string('addcriterion', 'gradingform_dukrub');
            ...
            $dukrubtemplate .= html_writer::tag('div', $input, array('class' => 'addcriterion'));
        }
        $dukrubtemplate .= $this->dukrub_edit_options($mode, $options);
        $dukrubtemplate .= html_writer::end_tag('div');

        return str_replace('{NAME}', $elementname, $dukrubtemplate);
    }

    /**
     * Generates html template to view/edit the dukrub options. Expression {NAME} is used in
     ...
     * @return string
     */
    protected function dukrub_edit_options($mode, $options) {
        if ($mode != gradingform_dukrub_controller::DISPLAY_EDIT_FULL
                && $mode != gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN
                && $mode != gradingform_dukrub_controller::DISPLAY_PREVIEW) {
            // Options are displayed only for people who can manage
            return;
        }
        $html = html_writer::start_tag('div', array('class' => 'options'));
        $html .= html_writer::tag('div', get_string('dukruboptions', 'gradingform_dukrub'), array('class' => 'optionsheading'));
        ...
        foreach ($options as $option => $value) {
            ...
            switch ($option) {
                case 'sortlevelsasc':
                    ...
                    $html .= html_writer::label(get_string($option, 'gradingform_dukrub'), $attrs['id'], false, array('class' => 'label'));
                    $value = (int)(!!$value); // make sure $value is either 0 or 1
                    if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FULL) {
                        $selectoptions = array(0 => get_string($option.'0', 'gradingform_dukrub'), 1 => get_string($option.'1', 'gradingform_dukrub'));
                        ...
                        $html .= html_writer::tag('span', get_string($option.$value, 'gradingform_dukrub'), array('class' => 'value'));
                        if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN) {
                    ...
                    break;
                default:
                    if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN && $value) {
                    ...
                    if ($mode == gradingform_dukrub_controller::DISPLAY_EDIT_FROZEN || $mode == gradingform_dukrub_controller::DISPLAY_PREVIEW) {
                    ...
                    $html .= html_writer::tag('label', get_string($option, 'gradingform_dukrub'), array('for' => $attrs['id']));
                    break;
            }
            if (get_string_manager()->string_exists($option.'_help', 'gradingform_dukrub')) {
                $html .= $this->help_icon($option, 'gradingform_dukrub');
            }
    ...
    /**
     * This function returns html code for displaying dukrub. Depending on $mode it may be the code
     * to edit dukrub, to preview the dukrub, to evaluate somebody or to review the evaluation.
     *
     * It is very unlikely that this function needs to be overriden by theme. It does not produce
     * any html code, it just prepares data about dukrub design and evaluation, adds the CSS
     * class to elements and calls the functions level_template, criterion_template and
     * dukrub_template
     *
     * @param array $criteria data about the dukrub design
     * @param array $options display options for this dukrub, defaults are: {@link gradingform_dukrub_controller::get_default_options()}
     * @param int $mode dukrub display mode, see {@link gradingform_dukrub_controller}
     * @param string $elementname the name of the form element (in editor mode) or the prefix for div ids (in view mode)
     ...
     * @return string
     */
    public function display_dukrub($criteria, $options, $mode, $elementname = null, $values = null) {
        $criteriastr = '';
        $cnt = 0;
        foreach ($criteria as $id => $criterion) {
            ...
            foreach ($criterion['levels'] as $levelid => $level) {
                ...
                if ($level['checked'] && ($mode == gradingform_dukrub_controller::DISPLAY_EVAL_FROZEN || $mode == gradingform_dukrub_controller::DISPLAY_REVIEW || $mode == gradingform_dukrub_controller::DISPLAY_VIEW)) {
                    $level['class'] .= ' checked';
                ...
                if ($level['checked'] && is_array($criterionvalue['leveloverwrite'])) {
                   ...
                   $level['leveloverwrite'] = $criterionvalue['leveloverwrite'][$levelid];
                    ...
                    $level['leveloverwrite'] = $criterionvalue['leveloverwrite'];
                } else {
                    // Else we use default score.
                    $level['leveloverwrite'] = $level['score'];
                }
        ...
        return $this->dukrub_template($mode, $options, $elementname, $criteriastr);
    }
     ...
     *
     * @param array $instances array of objects of type gradingform_dukrub_instance
     ...
     *
     * @param gradingform_dukrub_instance $instance
     ...
     */
    public function display_instance(gradingform_dukrub_instance $instance, $idx, $cangrade) {
        $criteria = $instance->get_controller()->get_definition()->dukrub_criteria;
        $options = $instance->get_controller()->get_options();
        $values = $instance->get_dukrub_filling();
        if ($cangrade) {
            $mode = gradingform_dukrub_controller::DISPLAY_REVIEW;
            $showdescription = $options['showdescriptionteacher'];
        } else {
            $mode = gradingform_dukrub_controller::DISPLAY_VIEW;
            $showdescription = $options['showdescriptionstudent'];
        }
        $output = '';
        if ($showdescription) {
            $output .= $this->box($instance->get_controller()->get_formatted_description(), 'gradingform_dukrub-description');
        }
        $output .= $this->display_dukrub($criteria, $options, $mode, 'dukrub'.$idx, $values);
        return $output;
    }
    ...
    public function display_regrade_confirmation($elementname, $changelevel, $value) {
        $html = html_writer::start_tag('div', array('class' => 'gradingform_dukrub-regrade', 'role' => 'alert'));
        if ($changelevel<=2) {
            $html .= html_writer::label(get_string('regrademessage1', 'gradingform_dukrub'), 'menu' . $elementname . 'regrade');
            $selectoptions = array(
                0 => get_string('regradeoption0', 'gradingform_dukrub'),
                1 => get_string('regradeoption1', 'gradingform_dukrub')
            );
            $html .= html_writer::select($selectoptions, $elementname.'[regrade]', $value, false);
        } else {
            $html .= get_string('regrademessage5', 'gradingform_dukrub');
    ...
    /**
     * Generates and returns HTML code to display information box about how dukrub score is converted to the grade
     *
     * @param array $scores
     * @return string
     */
    public function display_dukrub_mapping_explained($scores) {
        $html = '';
        if (!$scores) {
            return $html;
        }
        if ($scores['minscore'] <> 0) {
            $html .= $this->output->notification(get_string('zerolevelsabsent', 'gradingform_dukrub'), 'error');
        }
        $html .= $this->output->notification(get_string('dukrubmappingexplained', 'gradingform_dukrub', (object)$scores), 'info');
        return $html;
    }
}
