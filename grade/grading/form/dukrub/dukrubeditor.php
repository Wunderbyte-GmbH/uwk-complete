...
/**
 * File contains definition of class MoodleQuickForm_dukrubeditor
 *
...
/**
 * Form element for handling dukrub editor
 *
 * The dukrub editor is defined as a separate form element. This allows us to render
 * criteria, levels and buttons using the dukrub's own renderer. Also, the required
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class MoodleQuickForm_dukrubeditor extends HTML_QuickForm_input {
    ...
    /** @var bool Message to display in front of the editor (that there exist grades on this dukrub being edited) */
    protected $regradeconfirmation = false;

    /**
     * Constructor for dukrub editor
     *
     ...
     */
    public function MoodleQuickForm_dukrubeditor($elementName=null, $elementLabel=null, $attributes=null) {
    ...
    /**
     * Specifies that confirmation about re-grading needs to be added to this dukrub editor.
     * $changelevel is saved in $this->regradeconfirmation and retrieved in toHtml()
     *
     * @see gradingform_dukrub_controller::update_or_check_dukrub()
    ...
    public function toHtml() {
        global $PAGE;
        $html = $this->_getTabs();
        $renderer = $PAGE->get_renderer('gradingform_dukrub');
        $data = $this->prepare_data(null, $this->wasvalidated);
        if (!$this->_flagFrozen) {
            $mode = gradingform_dukrub_controller::DISPLAY_EDIT_FULL;
            $module = array('name'=>'gradingform_dukrubeditor', 'fullpath'=>'/grade/grading/form/dukrub/js/dukrubeditor.js',
                'requires' => array('base', 'dom', 'event', 'event-touch', 'escape'),
                'strings' => array(array('confirmdeletecriterion', 'gradingform_dukrub'), array('confirmdeletelevel', 'gradingform_dukrub'),
                    array('criterionempty', 'gradingform_dukrub'), array('levelempty', 'gradingform_dukrub')
                ));
            $PAGE->requires->js_init_call('M.gradingform_dukrubeditor.init', array(
            ...
            // Dukrub is frozen, no javascript needed
        ...
        $html .= $renderer->display_dukrub($data['criteria'], $data['options'], $mode, $this->getName());
        return $html;
    }
     ...
     * - if options not passed (i.e. we create a new dukrub) fills the options array with the default values
    ...
    protected function prepare_data($value = null, $withvalidation = false) {
        ...
        $totalscore = 0;
        $errors = array();
        $return = array('criteria' => array(), 'options' => gradingform_dukrub_controller::get_default_options());
        ...
        if ($withvalidation) {
            ...
            if (count($errors)) {
                ...
                foreach ($errors as $error => $v) {
                    $rv[] = get_string($error, 'gradingform_dukrub');
                }
    ...
    /**
     * Validates that dukrub has at least one criterion, at least two levels within one criterion,
