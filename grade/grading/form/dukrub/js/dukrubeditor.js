M.gradingform_dukrubeditor = {'templates' : {}, 'eventhandler' : null, 'name' : null, 'Y' : null};

/**
 * This function is called for each dukrubeditor on page.
 */
M.gradingform_dukrubeditor.init = function(Y, options) {
    M.gradingform_dukrubeditor.name = options.name
    M.gradingform_dukrubeditor.Y = Y
    M.gradingform_dukrubeditor.templates[options.name] = {
        'criterion' : options.criteriontemplate,
        'level' : options.leveltemplate
    }
    M.gradingform_dukrubeditor.disablealleditors()
    Y.on('click', M.gradingform_dukrubeditor.clickanywhere, 'body', null)
    YUI().use('event-touch', function (Y) {
        Y.one('body').on('touchstart', M.gradingform_dukrubeditor.clickanywhere);
        Y.one('body').on('touchend', M.gradingform_dukrubeditor.clickanywhere);
    })
    M.gradingform_dukrubeditor.addhandlers()
};

// Adds handlers for clicking submit button. This function must be called each time JS adds new elements to html
M.gradingform_dukrubeditor.addhandlers = function() {
    var Y = M.gradingform_dukrubeditor.Y
    var name = M.gradingform_dukrubeditor.name
    if (M.gradingform_dukrubeditor.eventhandler) M.gradingform_dukrubeditor.eventhandler.detach()
    M.gradingform_dukrubeditor.eventhandler = Y.on('click', M.gradingform_dukrubeditor.buttonclick, '#dukrub-'+name+' input[type=submit]', null);
}

// switches all input text elements to non-edit mode
M.gradingform_dukrubeditor.disablealleditors = function() {
    var Y = M.gradingform_dukrubeditor.Y
    var name = M.gradingform_dukrubeditor.name
    Y.all('#dukrub-'+name+' .level').each( function(node) {M.gradingform_dukrubeditor.editmode(node, false)} );
    Y.all('#dukrub-'+name+' .description').each( function(node) {M.gradingform_dukrubeditor.editmode(node, false)} );
}

// function invoked on each click on the page. If level and/or criterion description is clicked
// it switches this element to edit mode. If dukrub button is clicked it does nothing so the 'buttonclick'
// function is invoked
M.gradingform_dukrubeditor.clickanywhere = function(e) {
    ...
    if (el) {
        if (el.one('textarea').hasClass('hiddenelement')) {
            M.gradingform_dukrubeditor.disablealleditors()
            M.gradingform_dukrubeditor.editmode(el, true, focustb)
        }
        return
    }
...
M.gradingform_dukrubeditor.editmode = function(el, editmode, focustb) {
    ...
    if (!taplain) {
        ...
        taplain = ta.get('parentNode').one('.plainvalue')
        taplain.one('.pseudotablink').on('focus', M.gradingform_dukrubeditor.clickanywhere)
        if (tb) {
            ...
            tbplain.one('.pseudotablink').on('focus', M.gradingform_dukrubeditor.clickanywhere)
        }
    }
    ...
    if (!editmode) {
        ...
        if (value.length) taplain.removeClass('empty')
            ...
            value = (el.hasClass('level')) ? M.util.get_string('levelempty', 'gradingform_dukrub') : M.util.get_string('criterionempty', 'gradingform_dukrub')
...
// handler for clicking on submit buttons within dukrubeditor element. Adds/deletes/rearranges criteria and/or levels on client side
M.gradingform_dukrubeditor.buttonclick = function(e, confirmed) {
    var Y = M.gradingform_dukrubeditor.Y
    var name = M.gradingform_dukrubeditor.name
    ...
    if (chunks.length>4 || action == 'addlevel') {
        elements_str = '#dukrub-'+name+' #'+name+'-criteria-'+chunks[2]+'-levels .level'
    } else {
        elements_str = '#dukrub-'+name+' .criterion'
    }
    // prepare the id of the next inserted level or criterion
    var newlevid = 0;
    var newid = 0;
    if (action == 'addcriterion' || action == 'addlevel' || action == 'duplicate' ) {
        newid = M.gradingform_dukrubeditor.calculatenewid('#dukrub-'+name+' .criterion');
        newlevid = M.gradingform_dukrubeditor.calculatenewid('#dukrub-'+name+' .level');
    }
    var dialog_options = {
        'scope' : this,
        'callbackargs' : [e, true],
        'callback' : M.gradingform_dukrubeditor.buttonclick
    };
    if (chunks.length == 3 && action == 'addcriterion') {
        ...
        for (levidx=0;levidx<levelsscores.length;levidx++) {
            levelsstr += M.gradingform_dukrubeditor.templates[name].level.
                replace(/\{LEVEL-id\}/g, 'NEWID'+(newlevid+levidx)).
                replace(/\{LEVEL-score\}/g, levelsscores[levidx]).
                replace(/\{LEVEL-index\}/g, levidx + 1);
        }
        var newcriterion = M.gradingform_dukrubeditor.templates[name]['criterion'].replace(/\{LEVELS\}/, levelsstr)
        parentel.append(newcriterion.replace(/\{CRITERION-id\}/g, 'NEWID'+newid).replace(/\{.+?\}/g, ''))
        M.gradingform_dukrubeditor.assignclasses('#dukrub-'+name+' #'+name+'-criteria-NEWID'+newid+'-levels .level')
        M.gradingform_dukrubeditor.addhandlers();
        M.gradingform_dukrubeditor.disablealleditors()
        M.gradingform_dukrubeditor.assignclasses(elements_str)
        M.gradingform_dukrubeditor.editmode(
            Y.one('#dukrub-' + name + ' #' + name + '-criteria-NEWID' + newid + '-description-cell'), true
        );
        ...
        var newlevel = M.gradingform_dukrubeditor.templates[name]['level'].
        ...
        M.gradingform_dukrubeditor.disablealleditors()
        M.gradingform_dukrubeditor.assignclasses(elements_str)
        M.gradingform_dukrubeditor.editmode(parent.all('.level').item(parent.all('.level').size()-1), true)
        ...
        if (confirmed) {
            ...
            dialog_options['message'] = M.util.get_string('confirmdeletecriterion', 'gradingform_dukrub')
        ...
        var newcriterion = M.gradingform_dukrubeditor.templates[name].criterion
        ...
        M.gradingform_dukrubeditor.disablealleditors();
        M.gradingform_dukrubeditor.assignclasses(elements_str);
        M.gradingform_dukrubeditor.editmode(Y.one('#dukrub-'+name+' #'+name+'-criteria-NEWID'+newid+'-description-cell'),true);
        ...
        if (confirmed) {
            ...
            dialog_options['message'] = M.util.get_string('confirmdeletelevel', 'gradingform_dukrub')
            M.util.show_confirm_dialog(e, dialog_options);
        }
    } else {
        // unknown action
...
M.gradingform_dukrubeditor.assignclasses = function (elements_str) {
    var elements = M.gradingform_dukrubeditor.Y.all(elements_str)
...
M.gradingform_dukrubeditor.calculatenewid = function (elements_str) {
    var newid = 1
    M.gradingform_dukrubeditor.Y.all(elements_str).each( function(node) {
